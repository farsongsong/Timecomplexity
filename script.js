document.addEventListener("DOMContentLoaded", () => {
  ...
  function generateArray(){
    ...
    cmpCount=0;
    initCharts();
    statusDiv.innerText="현재 알고리즘: 없음";
  }
  ...
  function initCharts(){
    algoList.forEach((algo,i)=>{
      const canvas = document.getElementById(`chart_${algo}`);
      if(!canvas) return;
  ...
  async function bubbleSort(a){
    ...
    statusDiv.innerText="버블 정렬 실행 중...";
    ...
    statusDiv.innerText=`버블 정렬 완료 | 비교:${cmpCount} | 시간:${(performance.now()-startTime).toFixed(2)}ms`;
  }

  async function selectionSort(a){
    a=a.slice(); cmpCount=0; startTime=performance.now();
    currentColor=algoColors["selection"];
    statusDiv.innerText="선택 정렬 실행 중...";
    for(let i=0;i<a.length-1;i++){
      let min=i;
    ...
    array=a.slice(); drawArray([],currentColor);
    statusDiv.innerText=`선택 정렬 완료 | 비교:${cmpCount} | 시간:${(performance.now()-startTime).toFixed(2)}ms`;
  }

  async function insertionSort(a){
    a=a.slice(); cmpCount=0; startTime=performance.now();
    currentColor=algoColors["insertion"];
    statusDiv.innerText="삽입 정렬 실행 중...";
    ...
    array=a.slice(); drawArray([],currentColor);
    statusDiv.innerText=`삽입 정렬 완료 | 비교:${cmpCount} | 시간:${(performance.now()-startTime).toFixed(2)}ms`;
  }

  async function quickSort(a){
    a=a.slice(); cmpCount=0; startTime=performance.now();
    currentColor=algoColors["quick"];
    statusDiv.innerText="퀵 정렬 실행 중...";
    ...
    array=a.slice(); drawArray([],currentColor);
    statusDiv.innerText=`퀵 정렬 완료 | 비교:${cmpCount} | 시간:${(performance.now()-startTime).toFixed(2)}ms`;
  }

  async function mergeSort(a){
    a=a.slice(); cmpCount=0; startTime=performance.now();
    currentColor=algoColors["merge"];
    statusDiv.innerText="병합 정렬 실행 중...";
    ...
    await ms(0,a.length);
    array=a.slice(); drawArray([],currentColor);
    statusDiv.innerText=`병합 정렬 완료 | 비교:${cmpCount} | 시간:${(performance.now()-startTime).toFixed(2)}ms`;
  }
