...
function initCharts(){
  algoList.forEach(algo=>{
    const ctx=document.getElementById(`chart-${algo}`).getContext("2d");
    if(charts[algo]) charts[algo].destroy();
    charts[algo] = new Chart(ctx,{
      type:"line",
      data:{datasets:[{label:"비교 횟수",data:[],borderColor:algoColors[algo],fill:false,pointRadius:2,tension:0.2}]},
      options:{
        responsive:true, animation:false,
        plugins:{legend:{display:true}},
        scales:{x:{type:"linear",title:{display:true,text:"시간(ms)"}},y:{beginAtZero:true,title:{display:true,text:"비교 횟수"}}}
      }
    });
  });
}
